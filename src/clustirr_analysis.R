Sys.setenv(PATH = paste(Sys.getenv("PATH"), "path_to_blastp/bin", sep=.Platform$path.sep))
library(ClustIRR)
dir.create("results/")
dir.create("results/clustirr_90")

# 1. prepare data
t <- get(load(file = "data/TCRb.RData"))
...
save(c, file = "results/clustirr_90/c.RData")
cat("DONE!\n")
...
save(dc, file = "results/clustirr_90/dc.RData", compress = TRUE)
cat("DONE!\n")
graph <- dc$graph
save(graph, file = "results/clustirr_90/graph.RData", compress = TRUE)
cat("DONE!\n")


# 4. get concise summary from dc 
dc_light <- list(com = dc$community_occupancy_matrix, ns = dc$node_summary, cs = dc$community_summary)
save(dc_light, file = "results/clustirr_90/dc_light.RData", compress = TRUE)


# 5. perform differential community occupancy for each patient
dir.create("results/clustirr_90/dco/", recursive = T)
...
lapply(X = pts, com, FUN = function(x, com) {
  ...
  if(!file.exists(paste0("results/clustirr_90/dco/ps_", x, ".RData"))) {
    ...
    save(d, file = paste0("results/clustirr_90/dco/", x, ".RData"))
    ps <- d$posterior_summary
    save(ps, file = paste0("results/clustirr_90/dco/ps_", x, ".RData"))
    save(cs, file = paste0("results/clustirr_90/dco/cs_", x, ".RData"))
  }
})
